...
{else}
  <div class="crm-block">
    <h3><i class="crm-i fa-line-chart"></i> {ts domain='com.aghstrategies.idbsurvey'}See how your fundraising compares{/ts}</h3>
  ...
  <div class="crm-block crm-content-block crm-report-form-block">
    {* <div class="help">
      <h2><i class="crm-i fa-line-chart"></i> {ts domain='com.aghstrategies.idbsurvey'}See how your fundraising compares{/ts}</h2>

      <p>{ts domain='com.aghstrategies.idbsurvey'}Use this report to calculate information for the <a href="https://www.thirdspacestudio.com/idbproject/" target="_blank">Individual Donor Benchmark Survey</a> from <a href="https://www.thirdspacestudio.com/" target="_blank">Third Space Studio</a>. The survey is designed for US-based nonprofit organizations with revenues of $2 million or less to compare their fundraising against similar organizations.{/ts}</p>
      <p>{ts domain='com.aghstrategies.idbsurvey'}While there are some subjective questions on the survey and some questions about expenses and staffing&mdash;information that isn&rsquo;t typically stored in CiviCRM&mdash;<a href="https://aghstrategies.com/" target="_blank">AGH Strategies</a> developed this report to quickly calculate the answers to the questions about fundraising results.{/ts}</p>
      <h2><i class="crm-i fa-wrench"></i> {ts domain='com.aghstrategies.idbsurvey'}Configure this report{/ts}</h2>
      <p>{ts domain='com.aghstrategies.idbsurvey'}The only thing the system can&rsquo;t automatically know is what financial types you consider to be &ldquo;donations&rdquo; as opposed to service fees or other revenue.  In the form above, <strong>select the financial types you want to count, and click &ldquo;Display Answers for the Survey&rdquo;</strong>.{/ts}</p>
      <p>{ts domain='com.aghstrategies.idbsurvey'}The survey form is at <a href="https://www.thirdspacestudio.com/idbsurvey/" target="_blank">https://www.thirdspacestudio.com/idbsurvey/</a>, and the question numbers here correspond to those on the &ldquo;Your Results&rdquo; section.{/ts}</p>
    </div> *}
    {if $questions}
